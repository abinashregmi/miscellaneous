...
int quadraticProbingInsert(int hashTable[], int size, int value) {
    int index = value % size;
    int i = 1;
    while (hashTable[(index + i * i) % size] != -1) {
        i++;
    }
    hashTable[(index + i * i) % size] = value;
...
int main() {
    ...
    start = clock();
    for (int i = 0; i < size; i++) {
        quadraticProbingInsert(hashTableQuadratic, TABLE_SIZE, unsortedArr[i]);
    }
    end = clock();
    double quadraticProbingTime = (double)(end - start) / CLOCKS_PER_SEC;
    ...
    printf("Quadratic Probing Time: %.6f seconds\n", quadraticProbingTime);
